...
# ============================================================
# Pipeline: Lagrangiano Aumentado + FEM (Quadrupolo) + TuRBO
# ============================================================
...
# -- Problema físico -----------------------------------------
from quadrupolo import f, g, h, x0, x_to_params, verify_fem, exportar_resultados

# -- Framework Lagrangiano Aumentado -------------------------
...
def solver_turbo(
    ...
    """
    Retorna solver(func, x0) -> x_opt compatível com alg_lagran_aug.

    Nota sobre sinal: o LA minimiza func, o TuRBO maximiza internamente.
    Avaliamos -func(x) para converter minimização em maximização.
    """
...
def main():
    ...
    print()
    print(f"f* = {f_opt:.6f}")
    print(f"g* = [{g_opt[0]:.6f}, {g_opt[1]:.6f}]")
    print(f"g[0] sobreposição {'violada' if g_opt[0] > 1e-4 else 'ok'}")
    print(f"g[1] domínio      {'violada' if g_opt[1] > 1e-4 else 'ok'}")

    melhora = (Gopt - G0) / max(abs(G0), 1e-12) * 100
    print(f"\nMelhora do gradiente = {melhora:.2f}%")

    # Exportação
    print("\n[Passo 4] Exportação")
    exportar_resultados(x0,  solver=solver_int,  label="quadrupolo_inicial", pasta="results")
    exportar_resultados(x_opt, solver=solver_int, label="quadrupolo_otimo",   pasta="results")
